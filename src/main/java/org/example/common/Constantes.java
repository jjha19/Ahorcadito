package org.example.common;

public class Constantes {
    public static final String FICHERO = "Diccionario";
    public static final String FICHEROB = "copiaSeguridad";
    public static final String MENU = "Introduce opción: ";
    public static final String OPCIONESROL = """
            1. Jugar
            2. Gestionar Diccionario de Datos (Protegido)
            """;
    public static final String OPCIONESJUEGO = """
            1.Partida Nueva
            2.Cargar ultima partida
            """;
    public static final String OPCIONESADMIN = """
                                        1. Listar Diccionario Ordenado. \n
                                        2. Insertar un elemento al diccionario \n
                                        3. Modificar algún campo de un elemento del diccionario \n
                                        4. Eliminar un elemento del diccionario \n
                                        """;
    public static final String BIENVENIDA = """
             AAAAA  H   H  OOO  RRRR   CCCC    AAAAA  DDDD   OOO
            A     A H   H O   O R   R C       A     A D   D O   O
            AAAAAAA HHHHH O   O RRRR  C       AAAAAAA D   D O   O
            A     A H   H O   O R  R  C       A     A D   D O   O
            A     A H   H  OOO  R   R  CCCC   A     A DDDD   OOO
            
                 -  Bienvenido al juego del Ahorcado  -
            """;
    public static final String INSERTAR = "Introduzca id, palabra, categoria (Al insertar cada valor aprete INTRO)";
    public static final String SOLICITUD = "Por favor intriduzca la contraseña";
    public static final String VACIO = "El archivo está vacío. Por favor introduzca una contraseña nueva";
    public static final String DENEGADO = "Contraseña incorrecta. Acceso Denegado";
    public static final String PERMITIDO = "Contraseña correcta. Acceso Permitido";
}
